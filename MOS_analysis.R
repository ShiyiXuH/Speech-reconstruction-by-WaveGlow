...
input_dir = "/Users/apple/Library/CloudStorage/OneDrive-UniversityCollegeLondon/MOS_res/exp1_quality"
...
filenames = dir()[grep("^1.*.txt$", dir())]   # create variable with filenames of all text files in the working directory that start with "2" and end with ".txt"
...
for ( i in 1:num_files ) # iterate through all files
{
  ...
  vocoder_type = str_detect(condition,'f') * 1                                 # 0= bandpassed, 1=synthesized
...
#all_res$recognition_rate <- as.numeric(all_res$recognition_rate)
#all_res$condition <- factor(all_res$condition, 
#        levels = c("bin1", "bin2", "bin3", "bin4", "filtered_bin1_normal",
#                     "filtered_bin2_normal", "filtered_bin3_normal", "filtered_bin4_normal",
#                   "normal", "original"),
...
plot( 1:4, m[1:4], type="p", col="indianred2", pch=16, lty=1,, xlab="Condition", ylab="Mean opinion score", ylim=c(0,5.5), xlim=c(0.5,6.5),lwd=line_width, cex.axis=expansion, cex.lab =expansion, cex = expansion, xaxt="none" )
...
# 添加legend
legend(1,5, legend=c("WaveGlow", "Bandpass filter", "Original speech"), col=c("indianred2", "#00BFC4", "black"), pch=c(16, 17, 15), cex=0.9)
...
data_cleaned <- data_cleaned %>%
  mutate(subject = factor(subject),
         condition = factor(condition, 
                            ...
                            labels = c("band1", "band2", "band3", "band4")),
         vocoder_type = factor(vocoder_type, 
                               levels=c(0, 1), 
                               labels=c("WaveGlow","Bandpass filter"))
         )


mos_anova <- aov_car(recognition_rate ~ vocoder_type * condition + 
                       Error(subject/(vocoder_type*condition)), data = data_cleaned)
print(mos_anova)
...
library("emmeans")
em1 <- emmeans(mos_anova, c("vocoder_type", "condition"))
em1
...
# 合起来进行配对t检验
data_cleaned$group <- with(data_cleaned, interaction(vocoder_type, condition))
t2 <- pairwise.t.test(data_cleaned$recognition_rate, data_cleaned$group, paired = TRUE, p.adjust.method = "holm")
t2
